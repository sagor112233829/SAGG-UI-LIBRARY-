--[[
    ...
    FIXED: Uses Camera.ViewportSize instead of PlayerGui.AbsoluteSize
]]
...
function Window.new(config)
	...
	-- Use updated screenSize from Camera.ViewportSize
	local screenW = screenSize.X
	local screenH = screenSize.Y
	local windowW = math.min(screenW, scaleOffset(420))
	local windowH = math.min(screenH, scaleOffset(740))
	...
	expandBtn.MouseButton1Click:Connect(function()
		...
		if expanded then
			originalSize = mainContainer.Size
			originalPos = mainContainer.Position
			local fullSize = UDim2.new(0, screenW, 0, screenH)
	...
	-- Auto-resize on camera viewport change (FIXED)
	local function onViewportChange()
		...
		if not expanded and not minimized then
			windowW = math.min(screenW, scaleOffset(420))
			windowH = math.min(screenH, scaleOffset(740))
			mainContainer.Size = UDim2.new(0, windowW, 0, windowH)
			mainContainer.Position = UDim2.new(0.5, -windowW/2, 0.5, -windowH/2)
		end
	end
	
	Camera:GetPropertyChangedSignal("ViewportSize"):Connect(onViewportChange)
	...
	function self:SetTheme(newThemeName, custom)
		...
		-- For full theme update, components would need re-rendering (simplified)
	end
